...
class EPub3::Builder:ver<0.1.0> {
  ...
  method make (
    ...
    self!make-package( $oebps-dir, $content-body);
...
note "item: $item";
  ...
  #-----------------------------------------------------------------------------
  # Make package
  method !make-package( Str $oebps-dir, XML::Element $content-body) {
    ...
    $m = append-element(
      ...
      %( 'opf:scheme' => $!epub-attrs<id-scheme>,)
    );
