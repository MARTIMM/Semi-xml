...
### Benefits sofar
* XML is generated by the classes therefore external module XML can be kicked out. Might still need it to reverse engineer to an internal tree.
* Elements and attributes in the sxml namespace are not removed. They are filtered when xml is generated.
* Basic searching is implemented in the Node class. This means that XML::XPath is also not needed anymore. Some tests still use it.
* The object tree is build before external methods are called. This means that these methods generating sxml objects can search in the tree to insert objects to its proper place. This means that the remapping code is also not needed anymore.
