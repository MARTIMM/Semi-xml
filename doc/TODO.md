...
### Benefits sofar
* XML is generated by the classes therefore external module XML can be kicked out.
* Elements and attributes in the sxml namespace are not removed. They are filtered when xml is generated.
* Basic searching is implemented in the Node class. This means that XML::XPath is also not needed anymore.
* The object tree is build before external methods are called. This means that these methods generating sxml objects can search in the tree to insert objects on its proper place. This means that remapping is also not needed anymore.
